...
func execute_action(action_name: String):
	if not can_execute_action(action_name):
		emit_signal("action_executed", action_name, false, "Recursos insuficientes!")
		return
	for action in get_available_actions():
		if action.name == action_name:
...
func attempt_network_discovery(network_id: String):
	...
	if party_data.treasury < cost:
		emit_signal("action_executed", "Investigar Rede", false, "Custo de %d, não tem recursos suficientes." % cost)
		return
	var old_treasury = party_data.treasury
	...
	if randf() < discovery_chance:
		...
		emit_signal("action_executed", "Investigar Rede", true, "Sucesso! Descobriu a rede: %s" % network["name"])
	else:
